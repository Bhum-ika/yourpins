...
>
  {copied ? "Copied ✓" : "Share Pins 🔗"}
      ...
      <form onSubmit={handleAddSection} className="flex gap-2 mb-8">
        ...
        <button
          type="submit"
          className="cursor-pointer bg-black text-white rounded-full px-5 py-2 text-sm hover:bg-gray-800"
        >
          Add
        </button>
      </form>
        ...
        <div className="mb-10">
          ...
          <div className="flex flex-col gap-2">
            {starredLinks.map((link) => (
              ...
              <div
                key={link._id}
                className="bg-white border border-yellow-100 rounded-2xl px-5 py-3 flex justify-between items-center hover:shadow-md transition-shadow"
              >
                <div>
                ...
                <div className="flex items-center gap-3">
                  <span className="text-xs text-gray-300 bg-gray-50 rounded-full px-3 py-1">
                    {link.section?.name}
                  </span>
      ...
      <div className="grid grid-cols-2 gap-4">
        {sections.map((section, index) => {
          ...
          return (
            <div
              key={section._id}
              className="flex justify-between bg-white border rounded-2xl p-5 hover:shadow-md transition-shadow border-gray-100"
            >
